...
builder.Services.AddCors(options =>
{
    options.AddPolicy("AllowMyOrigin", policy =>
    {
        policy.WithOrigins(
                ...
                "http://localhost:5173" // ƒл€ локальной разработки
            )
            .AllowAnyHeader()
            .AllowAnyMethod()
            .AllowCredentials();
...
using (var scope = app.Services.CreateScope())
{
    ...
    var logger = services.GetRequiredService<ILogger<Program>>(); // ѕолучаем логгер
    try
    {
        ...
        context.Database.Migrate(); // Ёта команда создает таблицы и примен€ет все ожидающие миграции
...
{
    app.UseExceptionHandler("/Error"); // Ќастроить страницу /Error или middleware
    app.UseHsts();
}

// app.UseHttpsRedirection(); // «акомментируй, если HTTPS терминируетс€ на реверс-прокси (например, на Render)
app.UseRouting();

app.UseCors("AllowMyOrigin"); // ”бедись, что им€ политики совпадает с тем, что задано выше
