...
WORKDIR /src # Используем /src как рабочую директорию для исходников
...
RUN dotnet restore MySolution.sln # Предполагаем, что MySolution.sln находится в /src после COPY . .
...
EXPOSE 7295 # Или твой порт Kestrel
